...
class Potd(Cog):
    ...
    async def post(self):
        msg = await self.bot.get_channel(config.potd_planning).fetch_message(
            config.potd_toggle
        )
        ...
        for num in range(len(data) - 1, 1, -1):
            if data[num][10] == "pending" and data[num - 1][10] != "pending":
                ...
                msg2 = await self.bot.get_channel(config.problem_of_the_day).send(
                    post2 + post4 + post3
                )
            ...
            await self.bot.get_channel(config.problem_of_the_day).send(
                "Sorry we dont have any POTD today."
            )
    ...
    async def on_message(self, message: discord.Message):
        ...
        if "potd" in message.content:
            await message.channel.send(
                ...
                file=discord.File(os.getcwd() + r"/images/submit.png"),
            )
            await message.channel.send(
                "Then type number to which you are submiting and attach your solution as a single pdf. As shown below",
                file=discord.File(os.getcwd() + r"/images/potd.png"),
            )
    ...
    async def fetch(self, interaction: discord.Interaction, num: int):
        ...
        if interaction.channel_id == config.problem_of_the_day:
            ...
            if data[num][6] and (
                data[num][10] == "done"
                or interaction.channel_id in (config.potd_planning, config.potd_botspam)
            ):
                try:
                    data[num][3] = (
                        await self.bot.fetch_user(data[num][11].__int__())
                    ).name
                except:
                    pass
                post = f"**POTD {data[num][0]}**\n**{data[num][1]}**\n{data[num][6]}"
    ...
    async def solution(
        ...
        data = sheet.get()
        if link and (
            interaction.user.get_role(config.admin)
            or interaction.user.get_role(config.potd_creator)
        ):
            data[num][9] = link
            await interaction.response.send_message(
                "Successfully updated the solution to the sheet."
            )
            sheet.update(data)
            return
        else:
    ...
    async def submit(
        ...
        if error:
            ...
            try:
                ...
                print(e)
                mention = "@" + data[num][3]
            await self.bot.get_channel(config.potd_botspam).send(
    ...
    @app_commands.describe(
        question_links="Image attachement links for more then one link each link should be in newline."
    )
    @on_interaction
    async def upload(
        self,
        ...
        if not all("attachments" in k for k in question_links.splitlines()):
            await interaction.response.send_message(
                "You need an attachemnt link. mobile: link on share ; pc: open image in newtab and copy url."
            )
            return
        data = sheet.get()
        now = datetime.now()
        ...
        if interaction.channel_id in (config.potd_planning, config.potd_botspam):
            ...
            msg = (
                "Do you want to approve his/her requested note this can't be reversed."
            )
            await channel.send(f"PoTD requested by user {interaction.user}.")
            await interaction.response.send_message(
                "Your potd has been requested for approval."
            )
        num = len(data)
        to_append = [
            len(data),
    ...
    async def cog_app_command_error(
        ...
        await self.bot.get_channel(config.log2).send(
